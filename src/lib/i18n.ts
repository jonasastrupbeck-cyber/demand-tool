...
const translations = {
  ...
  'dashboard.touchesPerCase': {
    en: 'Touches per value demand',
    da: 'Registreringer pr. værdiskabende efterspørgsel',
    sv: 'Poster per värdeskapande efterfrågan',
    de: 'Einträge pro Wert-Nachfrage',
  },
  'dashboard.touchesPerCaseTitle': {
    en: 'Touches per value demand (XmR)',
    da: 'Registreringer pr. værdiskabende efterspørgsel (XmR)',
    sv: 'Poster per värdeskapande efterfrågan (XmR)',
    de: 'Einträge pro Wert-Nachfrage (XmR)',
  },
  'dashboard.stepsPerCaseTitle': {
    en: 'Blocks of work per value demand (XmR)',
    da: 'Arbejdsblokke pr. værdiskabende efterspørgsel (XmR)',
    sv: 'Arbetsblock per värdeskapande efterfrågan (XmR)',
    de: 'Arbeitsblöcke pro Wert-Nachfrage (XmR)',
  },
  'dashboard.stepTotal': {
    en: 'Total blocks',
    da: 'Blokke i alt',
  ...
  'dashboard.calcTouchesPerCase': {
    en: 'One point per value demand (one customer’s case) = the number of touches (saved work entries) on it, plotted in case-open order. Centre line = the mean; limits = mean ± 2.66 × the average moving range (an XmR chart). Choose a value step to count only touches up to that case’s last touch on the step.',
    da: 'Ét punkt pr. værdiskabende efterspørgsel (én kundes sag) = antal registreringer (gemte arbejdsposter) på den, vist i rækkefølge efter sagsåbning. Midterlinje = gennemsnit; grænser = gennemsnit ± 2,66 × gennemsnitligt glidende interval (et XmR-kort). Vælg et værditrin for kun at tælle registreringer frem til sagens sidste registrering på trinnet.',
    sv: 'En punkt per värdeskapande efterfrågan (en kunds ärende) = antal poster (sparade arbetsposter) på den, i ordning efter ärendets öppning. Mittlinje = medelvärde; gränser = medelvärde ± 2,66 × genomsnittligt glidande intervall (ett XmR-diagram). Välj ett värdesteg för att bara räkna poster fram till ärendets sista post på steget.',
    de: 'Ein Punkt pro Wert-Nachfrage (der Fall eines Kunden) = Anzahl der Einträge (gespeicherte Arbeitseinträge), in Reihenfolge der Fallöffnung. Mittellinie = Mittelwert; Grenzen = Mittelwert ± 2,66 × durchschnittliche gleitende Spannweite (XmR).',
  },
  'dashboard.calcStepsPerCase': {
    en: 'One point per value demand (one customer’s case) = the number of work blocks of the chosen type (or that type as a % of its blocks), in case-open order, with XmR limits (mean ± 2.66 × the average moving range). A block is one tagged block of work; choose a value step to count only blocks tagged to it.',
    da: 'Ét punkt pr. værdiskabende efterspørgsel (én kundes sag) = antal arbejdsblokke af den valgte type (eller typens andel i % af dens blokke), i rækkefølge efter sagsåbning, med XmR-grænser (gennemsnit ± 2,66 × gennemsnitligt glidende interval). En blok er én markeret arbejdsblok; vælg et værditrin for kun at tælle blokke markeret dertil.',
    sv: 'En punkt per värdeskapande efterfrågan (en kunds ärende) = antal arbetsblock av vald typ (eller typens andel i % av dess block), i ordning efter ärendets öppning, med XmR-gränser (medelvärde ± 2,66 × genomsnittligt glidande intervall). Ett block är ett märkt arbetsblock; välj ett värdesteg för att bara räkna block märkta dit.',
    de: 'Ein Punkt pro Wert-Nachfrage (der Fall eines Kunden) = Anzahl der Arbeitsblöcke des gewählten Typs (oder dessen Anteil in % seiner Blöcke), in Reihenfolge der Fallöffnung, mit XmR-Grenzen (Mittelwert ± 2,66 × durchschnittliche gleitende Spannweite).',
  },
  'dashboard.caseStatusLabel': {
    en: 'Cases',
    da: 'Sager',
    sv: 'Ärenden',
    de: 'Fälle',
  },
  'dashboard.peoplePerValueDemand': {
    en: 'People per value demand',
    da: 'Personer pr. værdiskabende efterspørgsel',
    sv: 'Personer per värdeskapande efterfrågan',
    de: 'Personen pro Wert-Nachfrage',
  },
  'dashboard.peoplePerValueDemandTitle': {
    en: 'People per value demand (XmR)',
    da: 'Personer pr. værdiskabende efterspørgsel (XmR)',
    sv: 'Personer per värdeskapande efterfrågan (XmR)',
    de: 'Personen pro Wert-Nachfrage (XmR)',
  },
  // Stat-tile N label for the value-demand XmR charts (one point = one value demand).
  'dashboard.valueDemandsN': {
    en: 'Value demands',
    da: 'Værdiskabende efterspørgsler',
    sv: 'Värdeskapande efterfrågan',
    de: 'Wert-Nachfragen',
  },
  'dashboard.calcPeoplePerValueDemand': {
    en: 'One point per value demand (one customer’s case) = the number of distinct people who worked on it, plotted in case-open order, with XmR limits (mean ± 2.66 × the average moving range). It counts the distinct names recorded on the case’s touches — the person who did the work where captured, otherwise whoever recorded the touch. So it reads as “how many people the customer’s demand passed through” only when each person’s work is attributed; otherwise it counts distinct recorders.',
    da: 'Ét punkt pr. værdiskabende efterspørgsel (én kundes sag) = antal forskellige personer, der har arbejdet på den, vist i rækkefølge efter sagsåbning, med XmR-grænser (gennemsnit ± 2,66 × gennemsnitligt glidende interval). Den tæller de forskellige navne, der er registreret på sagens registreringer — den person, der udførte arbejdet, hvor det er angivet, ellers den, der registrerede berøringen. Så den viser “hvor mange personer kundens efterspørgsel gik igennem” kun, når hver persons arbejde er tilskrevet; ellers tæller den forskellige registranter.',
    sv: 'En punkt per värdeskapande efterfrågan (en kunds ärende) = antal olika personer som arbetat på den, i ordning efter ärendets öppning, med XmR-gränser (medelvärde ± 2,66 × genomsnittligt glidande intervall). Den räknar de olika namn som registrerats på ärendets poster — personen som utförde arbetet där det angetts, annars den som registrerade beröringen. Så den visar “hur många personer kundens efterfrågan gick igenom” endast när varje persons arbete tillskrivs; annars räknar den olika registrerare.',
    de: 'Ein Punkt pro Wert-Nachfrage (der Fall eines Kunden) = Anzahl der verschiedenen Personen, die daran gearbeitet haben, in Reihenfolge der Fallöffnung, mit XmR-Grenzen (Mittelwert ± 2,66 × durchschnittliche gleitende Spannweite). Gezählt werden die verschiedenen auf den Einträgen des Falls erfassten Namen — die Person, die die Arbeit erledigt hat, sofern erfasst, sonst wer den Kontakt erfasst hat. Sie zeigt „durch wie viele Personen die Nachfrage des Kunden lief“ nur, wenn die Arbeit jeder Person zugeordnet ist; andernfalls zählt sie verschiedene Erfasser.',
  },
  ...
  'capture.caseArchiveTitle': {
    en: 'Archive this case?',
    da: 'Arkivér denne sag?',
    sv: 'Arkivera detta ärende?',
    de: 'Diesen Fall archivieren?',
  },
  'capture.caseArchiveBody': {
    en: 'Case #{ref} and its {n} touches will be hidden from every chart and count. It stays recoverable in Settings — nothing is deleted.',
    da: 'Sag #{ref} og dens {n} registreringer skjules fra alle diagrammer og optællinger. Den kan gendannes i Indstillinger — intet slettes.',
    sv: 'Ärende #{ref} och dess {n} poster döljs från alla diagram och räkningar. Det kan återställas i Inställningar — inget raderas.',
    de: 'Fall #{ref} und seine {n} Einträge werden aus allen Diagrammen und Zählungen ausgeblendet. In den Einstellungen wiederherstellbar — nichts wird gelöscht.',
  },
  'capture.caseArchiveConfirm': {
    en: 'Archive',
    da: 'Arkivér',
    sv: 'Arkivera',
    de: 'Archivieren',
  },
  'capture.caseArchiveCancel': {
    en: 'Cancel',
    da: 'Annullér',
    sv: 'Avbryt',
    de: 'Abbrechen',
  ...
  'settings.manageCasesTitle': {
    en: 'Manage cases',
    da: 'Administrér sager',
    sv: 'Hantera ärenden',
    de: 'Fälle verwalten',
  },
  'settings.manageCasesDesc': {
    en: 'Archive learning or practice cases so they don’t count in the data, or permanently delete them. Archived cases can be restored here.',
    da: 'Arkivér lærings- eller øvelsessager, så de ikke tæller med i data, eller slet dem permanent. Arkiverede sager kan gendannes her.',
    sv: 'Arkivera inlärnings- eller övningsärenden så att de inte räknas i data, eller radera dem permanent. Arkiverade ärenden kan återställas här.',
    de: 'Lern- oder Übungsfälle archivieren, damit sie nicht in die Daten zählen, oder dauerhaft löschen. Archivierte Fälle können hier wiederhergestellt werden.',
  },
  'settings.manageCasesEmpty': {
    en: 'No cases yet.',
    da: 'Ingen sager endnu.',
    sv: 'Inga ärenden ännu.',
    de: 'Noch keine Fälle.',
  },
  'settings.caseArchivedBadge': {
    en: 'Archived',
    da: 'Arkiveret',
    sv: 'Arkiverat',
    de: 'Archiviert',
  },
  'settings.caseRestore': {
    en: 'Restore',
    da: 'Gendan',
    sv: 'Återställ',
    de: 'Wiederherstellen',
  ...
  'settings.caseDeleteConfirm': {
    en: 'Tap again to delete',
    da: 'Tryk igen for at slette',
    sv: 'Tryck igen för att radera',
    de: 'Zum Löschen erneut tippen',
  },
  ...
  'capture.toggles.workedBy': {
    en: 'Who worked on each touch',
    da: 'Hvem arbejdede på hver berøring',
    sv: 'Vem arbetade på varje beröring',
    de: 'Wer an jedem Kontakt gearbeitet hat',
  },
