...
export async function getP2bsValueDemandLinks(
  ...
  const [caseRows, lpJunction, dtJunction, lpRows, vdRows] = await Promise.all([
    ...
    db.select({ id: lifeProblems.id, label: lifeProblems.label, sortOrder: lifeProblems.sortOrder })
      .from(lifeProblems).where(and(eq(lifeProblems.studyId, studyId), isNull(lifeProblems.archivedAt))),
...
export interface AskDeliveryRow {
  ...
  kind: 'amount' | 'number' | 'percent' | 'currency' | 'calculated' | 'date' | 'duration' | 'duration_months' | 'text' | 'choice' | 'multichoice';
